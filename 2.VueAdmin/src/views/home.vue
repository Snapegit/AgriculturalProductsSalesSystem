...
<div class="content" :style='{"padding":"20px 30px 20px","alignItems":"flex-start","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20231003/cc88bae96def417689c07dc1358d4a32.png) no-repeat center top / cover","display":"flex","fontSize":"14px","height":"100%"}'>
	...
	<div :style='{"border":"0px solid rgba(126, 96, 16, .1)","padding":"0px 0","margin":"0 0px 20px 0px","alignItems":"center","color":"#62779c","flexWrap":"wrap","background":"none","display":"flex","width":"100%","fontSize":"14px","order":"0"}'>
		<div :style='{"boxShadow":"0 0px 0px rgba(0,0,0,.3)","padding":"12px 0","margin":"0 2% 20px 0","borderColor":"rgba(126, 96, 16, .1)","alignItems":"center","textAlign":"left","flexDirection":"row-reverse","display":"flex","borderRadius":"8px","background":"url(http://codegen.caihongy.cn/20231003/b8ae010ba2ff44268dee3545917b55dd.png) no-repeat right bottom / 80% 80%,#fff","borderWidth":"0 0 0px","width":"18%","borderStyle":"solid"}' v-if="isAuth('zhunongshangpin','首页总数')">
			...
			<div :style='{"width":"auto","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				...
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"inherit","color":"inherit","height":"24px"}'>农产品总数</div>
			</div>
		</div>
	...
	<div class="type5" :style='{"alignContent":"flex-start","padding":"0","borderRadius":"8px","flexWrap":"wrap","background":"none","flex":"1","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"auto","order":"3"}'>
		<div id="zhunongshangpinChart1" class="echarts1" v-if="isAuth('zhunongshangpin','首页统计')"></div>
		<div id="zhunongshangpinChart2" class="echarts2" v-if="isAuth('zhunongshangpin','首页统计')"></div>
		<div id="zhunongtuopinChart1" class="echarts3" v-if="isAuth('zhunongtuopin','首页统计')"></div>
		<div id="zhunongtuopinChart2" class="echarts4" v-if="isAuth('zhunongtuopin','首页统计')"></div>
		<div id="nongjixuetangChart1" class="echarts5" v-if="isAuth('nongjixuetang','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	data() {
		return {
            zhunongshangpinCount: 0,
            zhunongtuopinCount: 0,
            nongjixuetangCount: 0,
	...
	mounted(){
		this.init();
		this.getzhunongshangpinCount();
		this.zhunongshangpinChat1();
		this.zhunongshangpinChat2();
		this.getzhunongtuopinCount();
		this.zhunongtuopinChat1();
		this.zhunongtuopinChat2();
		this.getnongjixuetangCount();
		this.nongjixuetangChat1();
	},
	methods:{
		// 词云
		...
		getzhunongshangpinCount() {
			this.$http({
				url: `zhunongshangpin/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.zhunongshangpinCount = data.data
				}
			})
		},
        ...
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                zhunongshangpinChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    zhunongshangpinChart1.resize();
                };
            }
        });
      })
      ...
      this.$nextTick(()=>{
        ...
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                zhunongshangpinChart2.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    zhunongshangpinChart2.resize();
                };
            }
        });
      })
		...
		getzhunongtuopinCount() {
			this.$http({
				url: `zhunongtuopin/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.zhunongtuopinCount = data.data
				}
			})
		},
		zhunongtuopinChat1() {
        ...
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                for(let i=0;i<res.length;i++){
                    ...
                    xAxis.push(res[i].wenzhangfenlei);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].wenzhangfenlei
                    })
                }
                var option = {};
                ...
                // 使用刚指定的配置项和数据显示图表。
                zhunongtuopinChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    zhunongtuopinChart1.resize();
                };
            }
        });
      })
      ...
      this.$nextTick(()=>{
        ...
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                zhunongtuopinChart2.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    zhunongtuopinChart2.resize();
                };
            }
        });
      })
		...
		getnongjixuetangCount() {
			this.$http({
				url: `nongjixuetang/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.nongjixuetangCount = data.data
				}
			})
		},
		nongjixuetangChat1() {
        ...
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                nongjixuetangChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    nongjixuetangChart1.resize();
                };
            }
        });
      })
