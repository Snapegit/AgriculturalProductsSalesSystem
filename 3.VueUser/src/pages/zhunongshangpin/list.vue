...
<div>
	...
	<div class="list-preview" :style='{"padding":"0 15%","margin":"0 auto","alignItems":"flex-start","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative"}'>
		...
		<div class="category-3" :style='{"padding":"20px","boxShadow":"0px 4px 10px 0px rgba(0,0,0,0.3)","margin":"0 30px 0 0","flexWrap":"wrap","background":"#fff","display":"flex","width":"250px","height":"auto","order":"3"}'>
			<div class="item" :class="swiperIndex == '-1' ? 'active' : ''" @click="getList(1, '全部')" :plain="isPlain">
				<div :style='{"color":"inherit","fontSize":"15px"}'>全部</div>
			</div>
			<div class="item" :class="swiperIndex == index ? 'active' : ''" v-for="(item, index) in fenlei" :key="index" @click="getList(1, item[feileiColumn], 'btn' + index)" :ref="'btn' + index" plain>
				<img v-if="item.image" :style='{"width":"60px","margin":"0 5px 0 0","objectFit":"cover","display":"block","height":"60px"}' :src="baseUrl + (item.image?item.image.split(',')[0]:'')">
				<div :style='{"color":"inherit","fontSize":"15px"}'>{{item[feileiColumn]}}</div>
			</div>
		</div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px 0 10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"0 10px 0px 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px 0 0","lineHeight":"42px","display":"inline-block"}'>产品名称：</div>
      ...
      <el-form-item :style='{"margin":"0 10px 0px 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px 0 0","lineHeight":"42px","display":"inline-block"}'>价格：</div>
	  ...
	  <el-button v-if="btnAuth('zhunongshangpin','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"#000","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/zhunongshangpinAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="sort_view" :style='{"width":"100%","padding":"10px 0","boxShadow":"0px 4px 10px 0px rgba(0,0,0,0.3)","margin":"20px auto","background":"#fff","order":"2"}'>
		<el-button :style='{"border":"0","padding":"0 15px","margin":"0 5px","borderRadius":"8px"}' @click="sortClick('price')">
			<span :style='{"margin":"0 2px 0 0","lineHeight":"40px","fontSize":"15px","color":"#000"}' class="icon iconfont icon-zhifudingjin" v-if="sortType!='price'"></span>
			<span :style='{"margin":"0 2px 0 0","lineHeight":"40px","fontSize":"15px","color":"#000"}' class="icon iconfont icon-jiantou23" v-else-if="sortType=='price'&&sortOrder=='desc'"></span>
			<span :style='{"margin":"0 2px 0 0","lineHeight":"40px","fontSize":"15px","color":"#000"}' class="icon iconfont icon-jiantou24" v-else-if="sortType=='price'&&sortOrder=='asc'"></span>
	...
	<div class="list" :style='{"width":"70%","margin":"0 auto 20px","background":"#fff","flex":"1","order":"4"}'>
		...
		<div class="list2 index-pv1" :style='{"padding":"0 0","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
			<div :style='{"padding":"25px","boxShadow":"0px 4px 10px 0px rgba(0,0,0,0.3)","margin":"10px 0","background":"#fff","display":"flex","width":"calc(50% - 10px)","fontSize":"0","position":"relative","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
				<div :style='{"border":"0","width":"220px","padding":"0","height":"110px"}'>
					<img @click.stop="imgPreView(item.tupian)" :style='{"border":"1px solid #eee","width":"100%","objectFit":"contain","background":"#fff","display":"block","height":"100%"}' v-if="item.tupian && item.tupian.substr(0,4)=='http'" :src="item.tupian.split(',')[0]" class="image" />
					<img @click.stop="imgPreView(baseUrl + (item.tupian?item.tupian.split(',')[0]:''))" :style='{"border":"1px solid #eee","width":"100%","objectFit":"contain","background":"#fff","display":"block","height":"100%"}' v-else :src="baseUrl + (item.tupian?item.tupian.split(',')[0]:'')" class="image" />
				</div>
				<div class="item-info" :style='{"padding":"10px","overflow":"hidden","alignItems":"center","flexWrap":"wrap","flex":"1","display":"flex","height":"auto"}'>
					<div :style='{"padding":"0 10px 0 0","whiteSpace":"nowrap","overflow":"hidden","color":"#000","width":"50%","lineHeight":"1","fontSize":"16px","textOverflow":"ellipsis","fontWeight":"bold","order":"1"}' class="name ">{{item.chanpinmingcheng}}</div>
					...
					<div :style='{"width":"50%","padding":"0 10px 0 0","order":"6"}'>
					  <span class="icon iconfont icon-geren16" :style='{"margin":"0 2px 0 0","lineHeight":"1.5","fontSize":"12px","color":"#666"}'></span>
					  <span :style='{"color":"#666","lineHeight":"1.5","fontSize":"12px"}'>{{item.nonghuzhanghao}}</span>
					</div>
					...
					<div :style='{"width":"30%","padding":"0 10px 0 0","order":"5"}'>
...
<style rel="stylesheet/scss" lang="scss" scoped>
	...
	.list .index-pv1 .animation-box:hover {
		transform: rotate(0deg) scale(1) skew(0deg, 2deg) translate3d(10px, 10px, 0px);
	...
	.list .index-pv1 .animation-box img:hover {
		transform: rotate(0deg) scale(1) skew(2deg, 0deg) translate3d(0px, 10px, 10px);
		-webkit-perspective: 1000px;
		perspective: 1000px;
		transition: 0.3s;
	}
